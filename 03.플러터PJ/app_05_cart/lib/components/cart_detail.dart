...
// 상태관리에서 변경부분 반영을 위한 build 메서드를
// 분리하여 반영하는 서브 클래스(그래서 언더바를 붙임)
class _CartDetailState extends State<CartDetail> {
  // 선택된 버튼 순번을 저장하는 변수
  int sequenceNum = 0;
  // ->> 이 클래스 내부에서 선언하여 사용하는 변수의
  // 값이 변경되면 이 변수를 사용하는 위젯이 자동 업데이트 된다!(중요!)
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        // 상품명+상품가격+별점+리뷰수+색상옵션+버튼 -> 별도구성해서 불러올 예정
        _buildCartInfo(),
      ],
    );
  }
...
//-> _buildSelectButton() 호출
  ...
  Widget _buildSelectButton(int seq, IconData mIcon) {
    // seq변수 : 버튼 클릭시 변경할 순번 변수값 셋팅
    // mIcon변수 : 버튼 아이콘
    return Container(
      width: 70,
      height: 70,
      decoration: BoxDecoration(
        // 버튼 배경 색상은 선택된것과 일반적인것으로 구분함
        // 현재 버튼하고 선택버튼 순번과 같으면 엑센트 색상 넣기
  ...
  Widget _buildCartInfo() {
    return Padding(
      ...
      child: Column(
        // 진행방향 수직축(crossAxis)의 시작부분부터 정렬(왼쪽끝)
  ...
  // 3. 색상옵션 위젯 만들기 메서드 : _buildOption()
  Widget _buildOption() {
    // 선택색상정보 변수에 셋업하기!
  ...
  Widget _buildDetailIcon(Color mColor) {
    return Padding(
      ...
      // 스택(Stack)위젯은 겹쳐지는 디자인을 할 때 사용함!
